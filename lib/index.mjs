...
var Router = class _Router {
  ...
  group(cb, options = {}) {
    ...
    const router = new _Router(
      newPrefix === "/" ? void 0 : validatePath(newPrefix),
      newMiddlewares
    );
    cb(router);
  ...
  applyRoutes(router) {
    ...
    Object.keys(router.routes).forEach((key) => {
      ...
      Object.assign(this.routes, {
        [key]: {
          controller: router.routes[key].controller,
          middlewares: router.routes[key].middlewares
        }
      });
    });
  }
...
(async () => {
  ...
  setInterval(async () => {
    const res = await client.post("/api/nested", {
      headers: {
...
export {
  Client_default as FlynestClient,
  Request,
  Response,
